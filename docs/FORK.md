---
title: Fork for another OPG
description: Copy Polje without inheriting Ivan’s ledger, tokens, or Cloudflare bindings.
---

# Fork Polje for another OPG

Polje is MIT. First instance is **OPG Ivan Jović** (`farm_id` / slug `ivan-jovic`) at [opg-ivanjovic.hr](https://opg-ivanjovic.hr). Public docs: [docs.opg-ivanjovic.hr](https://docs.opg-ivanjovic.hr). This file is how another family farm copies the stack without inheriting Ivan’s land ledger, tokens, or Cloudflare bindings.
...
| Custom domains `opg-ivanjovic.hr` / `docs.opg-ivanjovic.hr` | This farm’s public face |
...
2. New Cloudflare account (or a clean project): D1 database named `polje`, R2 bucket, KV namespace `polje` (binding `KV`), Queue `polje-ingest`, Durable Object class `FarmRuntime`. Optional Analytics Engine dataset `polje` (binding `METRICS`).
...
| `kv_namespaces[0].id` | live KV | new KV namespace |
